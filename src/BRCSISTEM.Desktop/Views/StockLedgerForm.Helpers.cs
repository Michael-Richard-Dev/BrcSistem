...
namespace BRCSISTEM.Desktop.Views
{
    public sealed partial class StockLedgerForm
    {
        ...
        {
            using (var dialog = new LookupSelectionForm("Selecionar Fornecedor", "Nome", _supplierOptions.Where(item => !string.IsNullOrWhiteSpace(item.Code)).ToArray()))
            {
        ...
        {
            ReloadLots();
            using (var dialog = new LookupSelectionForm("Selecionar Lote", "Nome", _lotOptions.Where(item => !string.IsNullOrWhiteSpace(item.Code)).ToArray()))
            {
                if (dialog.ShowDialog(this) == DialogResult.OK && dialog.SelectedOption != null)
                {
        ...
        {
            ReloadDependentReferences();
            using (var dialog = new LookupSelectionForm("Selecionar Almoxarifado", "Nome", _warehouseOptions.Where(item => !string.IsNullOrWhiteSpace(item.Code)).ToArray()))
            {
                if (dialog.ShowDialog(this) == DialogResult.OK && dialog.SelectedOption != null)
                {
