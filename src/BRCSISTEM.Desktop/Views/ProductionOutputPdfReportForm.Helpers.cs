...
{
    public sealed partial class ProductionOutputPdfReportForm
    {
        ...
        {
            _warehouseOptions = IncludeAllOption(ToWarehouseLookupOptions(_productionOutputReportController.LoadWarehousesForUser(_configuration, _databaseProfile, _identity.UserName)));
            BindCombo(_warehouseComboBox, _warehouseOptions, GetSelectedCode(_warehouseComboBox));
        }

        private void LoadProducts()
        {
            _productOptions = IncludeAllOption(ToProductLookupOptions(_productionOutputReportController.LoadProducts(_configuration, _databaseProfile)));
            BindCombo(_productComboBox, _productOptions, GetSelectedCode(_productComboBox));
        }
        ...
        private void QueryEntries()
        {
            try
            {
                _rows = _productionOutputReportController.SearchEntries(_configuration, _databaseProfile, BuildQuery());
        ...
        {
            var outputCount = _rows.Select(item => item.Number ?? string.Empty).Distinct(StringComparer.OrdinalIgnoreCase).Count();
            var consumed = _rows.Sum(item => item == null ? 0M : item.QuantityConsumed);
            _summaryLabel.Text = _rows.Length + " item(ns) encontrado(s) em " + outputCount + " saida(s) | Total consumido: "
                + consumed.ToString("N2", CultureInfo.GetCultureInfo("pt-BR"));
        }
        ...
        private void ClearFilters()
        {
            ...
            _outputNumberTextBox.Clear();
            _excludeCanceledCheckBox.Checked = false;
            _shiftComboBox.SelectedIndex = 0;
            SelectOptionByCode(_warehouseComboBox, _warehouseOptions, string.Empty);
            SelectOptionByCode(_productComboBox, _productOptions, string.Empty);
            QueryEntries();
        }
        ...
        {
            using (var dialog = new ProdutoSelecaoForm(_productOptions.Where(item => !string.IsNullOrWhiteSpace(item.Code)).ToArray()))
            {
                if (dialog.ShowDialog(this) == DialogResult.OK && dialog.SelectedOption != null)
                {
        ...
        private void ExportCsv()
        {
            if (_rows.Length == 0)
            {
                MessageBox.Show(this, "Nao ha dados para exportar.", "Relatorio de Saida de Producao", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            try
            {
                using (var dialog = new SaveFileDialog())
                {
                    ...
                    dialog.FileName = "relatorio_saida_producao_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".csv";
                    ...
                    {
                        writer.WriteLine("Saida;Data Saida;Almoxarifado;Produto;Material;Lote;Qtd Envio;Qtd Retorno;Qtd Consumida;Turno;Status");
                        foreach (var row in _rows)
                        {
                            writer.WriteLine(string.Join(";", new[]
                            {
                                ...
                                EscapeCsv(row.ProductDisplay),
                                EscapeCsv(row.MaterialDisplay),
                                EscapeCsv(row.LotDisplay),
                                EscapeCsv(row.QuantitySentText),
                                EscapeCsv(row.QuantityReturnedText),
                                EscapeCsv(row.QuantityConsumedText),
                                EscapeCsv(row.Shift),
                    ...
                    _productionOutputReportController.RegisterCsvExport(_configuration, _databaseProfile, _identity.UserName, BuildQuery(), _rows.Length);
                    SetStatus("Arquivo CSV exportado com sucesso.", false);
                    MessageBox.Show(this, "Dados exportados para:\n" + dialog.FileName, "Relatorio de Saida de Producao", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception exception)
            {
        ...
        {
            if (!(_grid.CurrentRow?.DataBoundItem is ProductionOutputReportEntry selectedRow))
            {
                MessageBox.Show(this, "Selecione uma saida de producao na lista para gerar o relatorio.", "Relatorio de Saida de Producao", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            try
            {
                var document = _productionOutputReportController.LoadDocument(_configuration, _databaseProfile, selectedRow.Number);
                using (var dialog = new SaveFileDialog())
                {
                    dialog.Title = "Salvar Relatorio PDF";
                    dialog.Filter = "PDF (*.pdf)|*.pdf|Todos os arquivos (*.*)|*.*";
                    dialog.DefaultExt = "pdf";
                    dialog.FileName = "relatorio_saida_producao_" + (selectedRow.Number ?? "sem_numero") + "_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".pdf";
                    if (dialog.ShowDialog(this) != DialogResult.OK)
                    {
                        return;
                    }

                    ProductionOutputPdfReportPdfExporter.Export(dialog.FileName, BuildFilterLines(), document, _identity.DisplayName);
                    _productionOutputReportController.RegisterPdfExport(_configuration, _databaseProfile, _identity.UserName, document.Number, document.Items.Length);
                    SetStatus("PDF gerado com sucesso.", false);

                    if (MessageBox.Show(this, "PDF salvo com sucesso.\n\nDeseja abrir agora?", "Relatorio de Saida de Producao", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
        ...
        private string[] BuildFilterLines()
        {
            return new[]
            {
                ...
                "Saida: " + (_outputNumberTextBox.Text ?? string.Empty),
                "Almoxarifado: " + GetSelectedDisplayText(_warehouseComboBox),
                "Produto: " + GetSelectedDisplayText(_productComboBox),
                "Turno: " + ((_shiftComboBox.SelectedItem as string) ?? "TODOS"),
        ...
        {
            if (!(_grid.Rows[e.RowIndex].DataBoundItem is ProductionOutputReportEntry row))
            {
                return;
            }
        ...
        private void NormalizeOutputNumberInput()
        {
            var normalized = (_outputNumberTextBox.Text ?? string.Empty).Replace(" ", string.Empty).ToUpperInvariant();
            if (!string.Equals(_outputNumberTextBox.Text, normalized, StringComparison.Ordinal))
            {
                var caret = normalized.Length;
                _outputNumberTextBox.Text = normalized;
                _outputNumberTextBox.SelectionStart = caret;
            }
        }

        private ProductionOutputReportQuery BuildQuery()
        {
            return new ProductionOutputReportQuery
            {
                StartDate = _startDateTextBox.Text,
                EndDate = _endDateTextBox.Text,
                OutputNumber = _outputNumberTextBox.Text,
                WarehouseCode = GetSelectedCode(_warehouseComboBox),
                ProductCode = GetSelectedCode(_productComboBox),
                Shift = (_shiftComboBox.SelectedItem as string) ?? string.Empty,
        ...
        private static LookupOption[] ToProductLookupOptions(ProductSummary[] items)
        {
            return (items ?? new ProductSummary[0]).Select(item => new LookupOption
            {
                Code = item.Code,
